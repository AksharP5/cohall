...
const chooseDevice = (
  ...
  Effect.gen(function* () {
    ...
    const matching = devices.find(
      (device) =>
        device.status !== "offline" && device.providers.includes(input.provider ?? "codex"),
    );
    if (matching !== undefined) {
      return matching.id;
    }

    const known = devices.find((device) => device.providers.includes(input.provider ?? "codex"));
    if (known !== undefined) {
      return known.id;
    }
    ...
    Effect.gen(function* () {
      ...
      switch (event._tag) {
        case "DeviceHeartbeat": {
          ...
          const device = (yield* store.listDevices()).find(
